          ...
          <>
            <div className="score-section">
              ...
              <div className="score-card score-main">
                <div className="score-label" style={{ color: 'var(--accent-jd)' }}>JD MATCH SCORE</div>
                <div className="score-value" style={{ color: 'var(--accent-jd)' }}>
                  {resume.jd_match.overall_score ?? '--'}
                </div>
                <div className="score-bar">
                  <div
                    className="score-bar-fill"
                    style={{ width: `${resume.jd_match.overall_score ?? 0}%`, background: 'var(--gradient-jd)' }}
                  />
                </div>
                <div className="score-explanation">
                  <span style={{ fontWeight: '600', color: 'var(--accent-jd)' }}>Target Role:</span> {resume.jd_match.role_title} {resume.jd_match.company ? `at ${resume.jd_match.company}` : ''}<br /><br />
                  ...
                  <div className="breakdown-item" key={idx}>
                    ...
                    <div className="breakdown-bar">
                      <div
                        ...
                        style={{ width: `${value !== '--' ? value : 0}%`, background: 'var(--accent-jd)' }}
                      />
                    </div>
                  </div>
                ))}
            ...
            <div className="two-column">
              <div className="section-card">
                <h3 style={{ fontSize: '0.85rem', letterSpacing: '0.1em', fontFamily: 'var(--font-mono)', color: 'var(--accent-jd)', marginBottom: '15px' }}>MATCHED KEYWORDS</h3>
                <div className="keyword-cloud">
                  {(resume.jd_match.matched_keywords ?? []).length > 0 ? (
                    (resume.jd_match.matched_keywords ?? []).map((kw, i) => (
                      <span className="keyword-tag" style={{ background: 'var(--success-badge-bg)', color: 'var(--success-badge-text)', border: '1px solid var(--success-badge-border)' }} key={i}>{kw}</span>
                    ))
                  ) : (
              ...
              <div className="section-card">
                <h3 style={{ fontSize: '0.85rem', letterSpacing: '0.1em', fontFamily: 'var(--font-mono)', color: 'var(--accent-jd)', marginBottom: '15px' }}>MISSING KEYWORDS</h3>
                <div className="keyword-cloud">
                  {(resume.jd_match.missing_keywords ?? []).length > 0 ? (
                    (resume.jd_match.missing_keywords ?? []).map((kw, i) => (
                      <span className="keyword-tag" style={{ background: 'var(--error-badge-bg)', color: 'var(--error-badge-text)', border: '1px solid var(--error-badge-border)' }} key={i}>{kw}</span>
                    ))
                  ) : (
